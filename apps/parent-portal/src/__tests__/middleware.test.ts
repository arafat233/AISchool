...
function createRequest(pathname: string, token?: string): NextRequest {
  const url = new URL(`http://localhost:3103${pathname}`);
...
describe('Parent Portal Middleware', () => {
  ...
  it('preserves the original path in the redirect query param', () => {
    const req = createRequest('/children');
    const res = middleware(req);
    const location = res.headers.get('location') || '';
    expect(location).toContain('from=%2Fchildren');
  });
});
