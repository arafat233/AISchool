...
describe('Parent Portal Login Page', () => {
  ...
  it('calls login API with email and password', async () => {
    (mockApi.post as jest.Mock).mockResolvedValueOnce({
      data: { accessToken: 'tok123', user: { id: '1', role: 'PARENT' } },
    });
    renderWithQuery(<LoginPage />);
    fireEvent.change(screen.getByLabelText(/email/i), { target: { value: 'parent@school.com' } });
    fireEvent.change(screen.getByLabelText(/password/i), { target: { value: 'Parent@123!' } });
    fireEvent.click(screen.getByRole('button', { name: /sign in/i }));
    await waitFor(() => {
      expect(mockApi.post).toHaveBeenCalledWith(
        expect.stringContaining('login'),
        expect.objectContaining({ email: 'parent@school.com', password: 'Parent@123!' }),
        expect.anything()
      );
    });
  });
  ...
  it('renders the portal title', () => {
    renderWithQuery(<LoginPage />);
    expect(screen.getByText(/AISchool Parent/i)).toBeInTheDocument();
  });
