  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">Overview for your child</p>
      </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {d.recentAnnouncements.map((a: any) => (
              <div key={a.id} className="bg-card rounded-xl border border-border px-4 py-3">
                <p className="text-sm font-medium text-foreground">{a.title}</p>
                <p className="text-xs text-muted-foreground mt-0.5">{new Date(a.publishedAt).toLocaleDateString()}</p>
              </div>
            ))}
          </div>
        </div>
