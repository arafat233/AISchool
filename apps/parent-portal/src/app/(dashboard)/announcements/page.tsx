...
export default function AnnouncementsPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-xl font-bold text-foreground">Announcements & Events</h1>

      <div className="bg-card rounded-xl border border-border overflow-hidden">
        <div className="px-5 py-4 border-b border-border/50 flex items-center gap-2">
          <Megaphone className="w-4 h-4 text-primary" />
          <h2 className="text-sm font-semibold text-foreground">School Announcements</h2>
        </div>
        <div className="divide-y divide-gray-100">
            ...
            <div key={a.id} className="px-5 py-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-1.5">{format(new Date(a.createdAt), "dd MMM yyyy, hh:mm a")}</p>
                </div>
                {a.attachmentUrl && (
          ...
          {!data?.length && <p className="px-5 py-8 text-sm text-muted-foreground text-center">No announcements</p>}
        </div>
      </div>
            ...
            {events.slice(0, 20).map((ev: any) => (
              <div key={ev.id} className="flex items-center gap-4 px-5 py-3">
                <div className="text-center w-12 shrink-0">
                  ...
                  <p className="text-xs text-muted-foreground">{format(new Date(ev.date), "MMM")}</p>
                </div>
                <div>
