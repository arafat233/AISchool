...
// Distinct subject colours to make the timetable easy to scan
const SUBJECT_COLORS: Record<string, string> = {
  Mathematics: "bg-blue-50 border-blue-100 text-blue-800",
  Science: "bg-green-50 border-green-100 text-green-800",
  English: "bg-purple-50 border-purple-100 text-purple-800",
  History: "bg-amber-50 border-amber-100 text-amber-800",
  Geography: "bg-teal-50 border-teal-100 text-teal-800",
  "Physical Education": "bg-orange-50 border-orange-100 text-orange-800",
  "Computer Science": "bg-cyan-50 border-cyan-100 text-cyan-800",
  Hindi: "bg-rose-50 border-rose-100 text-rose-800",
  Art: "bg-pink-50 border-pink-100 text-pink-800",
};
const DEFAULT_COLOR = "bg-gray-50 border-gray-100 text-gray-800";
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="bg-white rounded-xl border border-border p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {PERIODS.map((p) => {
              ...
              return (
                <div key={p} className={`px-3 py-1.5 rounded-lg border text-xs font-medium ${color}`}>
                  P{p} · {slot.subject}
                </div>
              );
            })}
        ...
        <table className="w-full text-xs min-w-[700px]">
          <thead>
            <tr className="border-b border-border bg-gray-50/50">
              <th className="text-left px-3 py-3 text-muted-foreground font-medium w-24">Period</th>
              {DAYS.map((day) => (
                <th key={day} className={cn(
                  ...
                  {day.slice(0, 3)}
                  {day === today && <span className="ml-1 bg-sidebar text-white rounded px-1 text-xs">Today</span>}
                </th>
              ))}
            </tr>
          </thead>
            ...
            {PERIODS.map((period) => (
              <tr key={period} className="border-b border-border/50">
                <td className="px-3 py-2.5 align-top">
                  ...
                  <div className="text-muted-foreground mt-0.5">{PERIOD_TIMES[period]}</div>
                </td>
                {DAYS.map((day) => {
                    ...
                    <td key={day} className={cn("px-2 py-2 align-top", isToday && "bg-sidebar/5")}>
                      {slot ? (
                        <div className={cn(
                          ...
                          <p className="font-semibold leading-none">{slot.subject}</p>
      ...
      <div className="flex items-start gap-2 flex-wrap">
        ...
        {subjects.map((s) => (
          <span key={s} className={`text-xs font-medium px-2.5 py-1 rounded-full border ${SUBJECT_COLORS[s] ?? DEFAULT_COLOR}`}>
            {s}
          </span>
        ))}
      </div>
