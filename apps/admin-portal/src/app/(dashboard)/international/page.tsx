  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">International School Modules</h1>
          <p className="text-sm text-gray-500 mt-1">IB PYP/MYP/DP · Cambridge IGCSE/A-Level · Foreign Students</p>
        </div>
      </div>
      ...
      {activeTab === "ib" && (
        <div className="space-y-4">
          <h2 className="font-semibold text-gray-800">IB DP Students — CAS Progress</h2>
          <div className="space-y-3">
            {MOCK_IB_STUDENTS.filter(s => s.programme === "DP").map(student => {
              ...
              return (
                <div key={student.name} className="bg-white border border-gray-200 rounded-xl p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <div className="font-medium text-gray-900">{student.name}</div>
                      <div className="text-xs text-gray-500">{student.programme} · {student.class}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-bold text-gray-900">{total}/150 hrs</div>
                      <div className="text-xs text-gray-400">Predicted: {student.predictedTotal} pts</div>
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    {(["creativity", "activity", "service"] as const).map(strand => {
                      ...
                      return (
                        <div key={strand} className={`rounded-lg p-2 ${hours >= req ? "bg-green-50" : "bg-gray-50"}`}>
                          <div className="text-xs text-gray-500 capitalize">{strand}</div>
                          <div className={`font-bold ${hours >= req ? "text-green-700" : "text-gray-900"}`}>{hours}/{req} hrs</div>
            ...
            <tbody className="divide-y divide-gray-100">
              {MOCK_EXPIRING_DOCS.map(doc => (
                <tr key={doc.name} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3 text-gray-600">{doc.nationality}</td>
                  <td className="px-5 py-3 text-gray-600">—</td>
                  <td className="px-5 py-3 text-gray-600">—</td>
                  <td className="px-5 py-3 text-gray-600">INR</td>
