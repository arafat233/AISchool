...
async def school_overview(school_id: str, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logger.error("School overview query failed: %s", exc, exc_info=True)
...
async def attendance_heatmap(
    ...
    try:
        ...
        logger.error("Attendance heatmap query failed: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error") from exc
...
async def finance_trend(school_id: str, months: int = 12, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logger.error("Finance trend query failed: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error") from exc
...
async def subject_performance(
    ...
    try:
        ...
        logger.error("Subject performance query failed: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error") from exc
...
async def class_performance(class_id: str, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logger.error("Class performance query failed: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error") from exc
