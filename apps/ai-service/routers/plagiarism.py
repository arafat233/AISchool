...
class PlagiarismCheckRequest(BaseModel):
    ...
    @classmethod
    def limit_text_size(cls, v: str) -> str:
        if len(v) > 50_000:
            raise ValueError("submission_text must not exceed 50,000 characters")
        return v
...
def extract_matching_spans(text1: str, text2: str, min_len: int = 8) -> list[str]:
    ...
    matches = []
    for n in range(min_len, min(4, min_len - 1), -1):
...
async def check_plagiarism(req: PlagiarismCheckRequest, db: AsyncSession = Depends(get_db)):
    try:
        ...
        try:
            ...
            logger.warning("TF-IDF vectorization failed, using zero similarities: %s", exc)
        ...
        logger.error("Plagiarism check failed: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail=f"Internal error during plagiarism detection: {exc}") from exc
...
async def class_plagiarism_report(assignment_id: str, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        try:
            ...
            logger.warning("TF-IDF vectorization failed for class report, returning empty pairs: %s", exc)
            return {"pairs": []}
        ...
        logger.error("Class plagiarism report failed: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail=f"Internal error during plagiarism detection: {exc}") from exc
